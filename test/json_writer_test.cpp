...
TEST(JsonWriterTest, UnicodeTest1) {
    auto json =
        R"(["6U閆崬밺뀫颒myj츥휘:$薈mY햚#rz飏+玭V㭢뾿愴YꖚX亥ᮉ푊\u0006垡㐭룝\"厓ᔧḅ^Sqpv媫\"⤽걒\"˽Ἆ?ꇆ䬔未tv{DV鯀Tἆl凸g\\㈭ĭ즿UH㽤"])"_json_set;
    auto it = find_by_element_name(json, "0");
    ASSERT_NE(json.end(), it);
    auto str = std::string{};
    detail::stringify(it->value<std::string>(), std::back_inserter(str));
    EXPECT_EQ(
        R"("6U閆崬밺뀫颒myj츥휘:$薈mY햚#rz飏+玭V㭢뾿愴YꖚX亥ᮉ푊\u0006垡㐭룝\"厓ᔧḅ^Sqpv媫\"⤽걒\"˽Ἆ?ꇆ䬔未tv{DV鯀Tἆl凸g\\㈭ĭ즿UH㽤")",
        str);
}
...
TEST(JsonWriterTest, JsonWriteTest2) {
    auto json = std::string{};
    write_json(static_cast<document>(builder("some value", element_type::int32_element,
                                             123)("some other value", element_type::boolean_element, true)),
...
std::string demangle(std::experimental::string_view t) {
...
TEST(JsonWriterTest, JsonWriteTest5) {
    ...
    write_json(static_cast<document>(builder("some oid", element_type::oid_element, oid)), std::back_inserter(json));
...
TEST(JsonWriterTest, JsonWriteTest7) {
    ...
    std::array<char, 12> oid{{static_cast<char>(0x50), static_cast<char>(0x7f), static_cast<char>(0x1f),
                              ...
                              static_cast<char>(0x6c), static_cast<char>(0xd7), static_cast<char>(0x99),
                              static_cast<char>(0x43), static_cast<char>(0x90), static_cast<char>(0x11)}};
    write_json(static_cast<document>(
                   builder("some ref", element_type::db_pointer_element, std::make_tuple("some collection", oid))),
               std::back_inserter(json));
    EXPECT_EQ(R"({ "some ref" : { "$ref" : "some collection", "$id" : { "$oid" : "507f1f77bcf86cd799439011" } } })",
              json);
}
...
TEST(JsonWriterTest, JsonWriteTest8) {
    ...
    //    EXPECT_EQ(R"([ "JSON Test Pattern pass1", { "object with 1 member" : [ "array with 1 element" ] }, {  }, [  ],
    //    -42, true, false, null, { "integer" : 1234567890, "real" : -9876.543210, "e" : 0.123456789e-12, "E ":
    //    1.234567890E+34, "" : 23456789012E66, "zero" : 0, "one" : 1, "space" : " ", "quote" : "\"", "backslash" :
    //    "\\", "controls" : "\b\f\n\r\t", "slash" : "/ & \/", "alpha" : "abcdefghijklmnopqrstuvwyz", "ALPHA" :
    //    "ABCDEFGHIJKLMNOPQRSTUVWYZ", "digit" : "0123456789", "0123456789" : "digit", "special" :
    //    "`1~!@#$%^&*()_+-={':[,]}|;.</>?", "hex" : "\u0123\u4567\u89AB\uCDEF\uabcd\uef4A", "true" : true, "false" :
    //    false, "null" : null, "array" : [  ], "object" : {  }, "address" : "50 St. James Street", "url" :
    //    "http://www.JSON.org/", "comment" : "// /* <!-- --", "# -- --> */" : " ", " s p a c e d " :[1,2 , 3 , 4 , 5 ,
    //    6 ,7 ],"compact":[1,2,3,4,5,6,7], "jsontext" : "{\"object with 1 member\":[ \"array with 1 element\" ] }",
    //    "quotes" : "&#34; \u0022 %22 0x22 034 &#x22;",
    //    "\/\\\"\uCAFE\uBABE\uAB98\uFCDE\ubcda\uef4A\b\f\n\r\t`1~!@#$%^&*()_+-=[]{}|;:',./<>?" : "A key can be any
    //    string" }, 0.5 ,98.6, 99.44, 1066, 1e1, 0.1e1, 1e-1, 1e00,2e+00,2e-00, "rosebud" ])",
    //              json_str);
